        ...
        if user_details and user_details.get('skills'):
            ...
            if isinstance(skills_data, str):
                try:
                    ...
                    print(f"Warning: Could not decode manual skills JSON for user {user_id}")
                    manual_skills = []
        ...
        if extracted_details and extracted_details.get('skills'):
            ...
            if isinstance(skills_data, str):
                try:
                    ...
                    print(f"Warning: Could not decode extracted skills JSON for user {user_id}")
                    extracted_skills = []
            elif isinstance(skills_data, list):
                extracted_skills = skills_data

        # Combine, deduplicate, title case, and sort skills
...
def fetch_jobs_from_api(query):
    ...
    querystring = {
        ...
        "job_requirements": "no_experience" # Stricter filter for students
    }
    headers = {
    ...
    try:
        ...
        # Optionally log e.response.text or e.response.status_code
        return []
    except Exception as e:
...
def search_jobs_multi_query_location():
    """
    Takes multiple base queries and locations, runs concurrent searches with
    'no_experience' filter, merges and deduplicates results.
    """
    token = request.cookies.get("token")
    if not token: return jsonify({"error": "Authentication required."}), 401
    try:
    ...
    # Split base queries and locations, removing empty entries
    ...
    # Adjust max_workers based on API limits and server resources if needed
    max_workers = min(len(combined_search_queries), 10) # Limit concurrent requests
    ...
    for job in all_job_data:
        ...
        if not job_id or job_id in processed_jobs: continue # Skip duplicates

        job_url = job.get('job_apply_link') or job.get('job_google_link') or job.get('job_linkedin_link')
        if not job_url: continue # Skip if no link
        ...
        formatted_job = {
            "job_id": job_id, # Include job_id for potential future use (e.g., saving jobs)
            ...
            "recommendation_reason": "Matched via multi-query/location search (entry-level focus)" # Updated reason
        }
        processed_jobs[job_id] = formatted_job # Use job_id as key for deduplication
    ...
    # General error Catcher (Specific errors handled in fetch_jobs_from_api)
    # except Exception as e:
    #     print(f"❌ Error during multi-location job search execution: {e}")
    #     traceback.print_exc()
    #     return jsonify({"error": "An internal server error occurred processing the job search."}), 500